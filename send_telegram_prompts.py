...
def send_prompt(job: dict, bot_token: str, chat_id: str) -> int:
    ...
    caption = (
        ...
        "<b>IMPORTANT — select video mode first:</b> In Gemini, tap <b>+</b> → "
        "<b>Video / Create video (Veo)</b>. Do not paste the prompt into normal Gemini Flash "
        "chat; Flash cannot generate the video and will refuse it.\n\n"
        "FULL PROMPT: open the attached .txt file, Select All → Copy, then paste only after "
        "Create video/Veo mode is active. Telegram does not allow a copy_text button for a prompt "
        "this long. Allow Gemini to open "

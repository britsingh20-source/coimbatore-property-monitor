...
# Channel-aware Meta credentials.  Real-estate remains the default so existing
# root-level social-ready uploads keep working without any migration.
CHANNEL_ENV = {
...
def validate_page_token(channel: str = "real_estate") -> dict:
    ...
    if resolved_page_id != page_id:
        raise RuntimeError(
            f"{channel}: Meta token cannot access configured Facebook Page: "
            f"resolved id={resolved_page_id or 'missing'}, configured page id={page_id}."
        )
...
def upload_to_r2(video_path: Path, video_id: str) -> tuple[str, str]:
    ...
    url = client.generate_presigned_url(
        "get_object", Params={"Bucket": bucket, "Key": key}, ExpiresIn=PRESIGNED_SECONDS
    )
    return key, url
...
def publish_instagram_reel(video_url: str, caption: str, channel: str = "real_estate") -> dict:
    ...
    for _ in range(POLL_ATTEMPTS):
        status = requests.get(
            f"{GRAPH}/{creation_id}/?fields=status_code,status&access_token={token}",
            timeout=30,
        )
...
def publish_facebook_reel(video_path: Path, caption: str, channel: str = "real_estate") -> dict:
    ...
    finish = requests.post(
        f"{GRAPH}/{page_id}/video_reels",
        data={"upload_phase": "finish", "video_id": video_id, "video_state": "PUBLISHED", "description": caption, "access_token": token},
        timeout=60,
    )

...
def _ai_clips(video_id: str) -> list[dict]:
    ...
    for scene_dir in sorted(path for path in root.iterdir() if path.is_dir()):
        ...
        clips.append({
            ...
            "provider": "AI representative visual",
...
def source_engaging_broll(job: dict) -> list[dict]:
    """Use only generated Tamil Nadu/Coimbatore representative visuals.

    This workflow intentionally fails closed. It never falls back to Pexels,
    Pixabay, other sellers' listing media, or advertiser stock when AI scenes
    are unavailable.
    """
    video_id = str(job["video_id"])
    ai = _ai_clips(video_id)
    if not ai:
        raise RuntimeError(f"No usable AI representative clips for {video_id}; stock fallback is disabled")
    saved = _copy_ai(video_id, ai)
    _write_attribution(video_id, saved)
    print(f"Engaging B-roll {video_id}: using {len(saved)} AI-generated Tamil Nadu/Coimbatore representative clips")
    return saved

...
class ReferenceFirstPromptTests(unittest.TestCase):
    def test_prompt_has_property_monitor_structure_and_brand(self):
        ...
        analysis = {"core_idea": "Corner mechanism", "system_type": "cabinet mechanism", "installation_method": "inside corner cabinet", "mechanism": "pull-out", "shot_subjects": ["wide kitchen"], "verified_facts": ["warm wood cabinets"], "forbidden_substitutes": ["ordinary shelf"], "highlighted_innovations": [{"name": "Corner pull-out", "why_notable": "uses dead corner", "installation_method": "inside cabinet", "mechanism": "pull-out linkage", "practical_benefit": "accessible storage", "visual_evidence": "trays move outward"}], "excluded_generic_scenes": ["ordinary dining table"]}
        ...
        self.assertIn("INNOVATION-SCOPE LOCK", prompt)
        self.assertIn("ordinary dining table", prompt)
